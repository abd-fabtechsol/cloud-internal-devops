...
export default ({fetchCandidateData,current})=>{
 let initialState={
  Interview:0,
  Submittal:0,
  Offer:0,
 ...
 //console.log(current,"curent overview");
  ...
  <Box className=""  sx={{ marginBottom: "20px", padding: "20px" }}>
    ...
    <Box component={Paper} className="pb-3"> 
    <div className="py-4 d-flex justify-content-between px-2 matches_box_bg">
         ...
         {candidateInfo&& <AddCandidateOverview fetchdata={fetchCandidateData} current={current} show={candidateInfo} onHide={()=>SetCandidateInfo(false)}/>}
          <Box className="px-3">
          {/* <div className='px-4'>
          ...
          <Box component={Paper} className=' mt-1 ' >
              <div className="py-2 d-flex justify-content-between px-4 matches_box_bg">
              <h6 className="text-center">
              About
              </h6>
           {type=="AG"||type=="CA"&&    <HiDotsVertical onClick={() => SetAbout(true)}/>}
              </div>
              <AddCandidateAbout fetchdata={fetchCandidateData} current={current} show={About} onHide={()=>SetAbout(false)}/>
              <Divider sx={{ m: 1 }} />
              <div className="px-3">
              <p className='fs_10'>
              General Comments
            ...
            <Box component={Paper}  className=' p-2 mt-1  '>
             
             <AddAdditionalInfoo fetchdata ={fetchCandidateData} current={current} show={candidateAddInfo} onHide={()=>SetCandidateAddInfo(false)}/>
    ...
    <Grid item sm={12} md={6}>
          ...
          <Box component={Paper}>
        {addMethod&&    <AddCommunication fetchdata={fetchCandidateData} item={communication} current={current} show={addMethod} onHide={()=>setAddMethod(false)}/>}
  ...
  {addLocation&&<AddAddress addresss={address}  fetchdata={fetchCandidateData} current={current} show={addLocation} onHide={()=>SetAddLocation(false)}/>}
