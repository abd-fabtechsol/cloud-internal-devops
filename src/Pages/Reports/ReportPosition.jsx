...
export default  ({user}) => {
  ...
  async function fetchData(page = 1) {
    ...
    const result = await request(`/ReportMatch/?page=${page}&type=${FilterData.type}&position_status=${FilterData.status}`);
    if (!result.ok) return;
    //console.log(result);
...
<PositionType size="small" value={FilterData.type} setValue={handleChange}/>
</div>
<div className="col-md-3">

<PositionStatus size="small" value={FilterData.status} setValue={handleChange}/>
</div>
<div className="col-md-3">
<AdminButton 
                        name="Filter"
...
const PositionStatus = ({value,setValue,...otherProps}) => {
...
const data=[
  {value:"OP",name:"OPEN"},
  {value:"PA",name:"PENDING_APPROVAL"},
  {value:"AP",name:"APPROVED"},
  {value:"OH",name:"ONHOLD"},
...
async function fetchCsvData(resultData,csvAPI){
  ...
  generateCSV(data,headers,"PositionReport")
  }